...
export class ActivityHelpComponent implements OnInit {
  ...
  constructor(
    ...
    this.activityType = this.workareaService.activities[this.workareaService.activities.length - 1].meta.type
  }
  ...
  ngOnInit() {
    ...
    this.workareaService.currentActivity$
      .subscribe(
        (activity: Activity) => {
          this.activityType = activity.meta.type
          this.getActivityHelp(this.activityType) },
        (err) => { console.log(err) }
      )
  }
  ...
  getActivityHelp(type: string) {
    if (!this.activityHelp[type]) {
      this.httpClient.get<string>(
        `${this.configService.params.api.v3.path}/help/${type}`
      ).subscribe(
